...
pub trait NginxHandlerCtxTrait {
    ...
    fn get_complex_value<T: FromStr>(
        ...
        if let Some(vs) = self.get_nginx_complex_value(&cv.cv) {
    ...
    fn get_nginx_complex_value(&mut self, cv: &Self::NginxComplexValueType) -> Option<&NgxStr>;
